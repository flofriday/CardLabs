...
fun builtinMul(
  ...
  var res: NumberValue = IntegerValue(1)
  for ((arg, loc) in args) {
    if (arg !is NumberValue) {
      throw SchemeError(
          "Unsupported Type",
          "Only numbers can be multiplied but one of the arguments was a ${arg.typeName()}",
          loc,
          null,
      )
    }
...
fun builtinDiv(
  ...
  val first = args.first().value
  if (first !is NumberValue) {
    throw SchemeError(
        "Unsuported Type",
        "Only numbers can be divided but one of the arguments was a ${args.first().value.typeName()}",
        args.first().location,
        null,
    )
  }
  var res: NumberValue = first

  for ((arg, loc) in args.drop(1)) {
    if (arg !is NumberValue) {
      throw SchemeError(
          "Unsupported Type",
          "Only numbers can be divided but one of the arguments was a ${arg.typeName()}",
          loc,
          null,
      )
    }
...
fun builtinEqual(
  ...
  for ((value, loc) in args) {
    if (value.javaClass != args[0].value::class.java) {
      throw SchemeError(
          "Expected same type", "Expected all arguments to be of the same type", loc, null)
    }
  }
