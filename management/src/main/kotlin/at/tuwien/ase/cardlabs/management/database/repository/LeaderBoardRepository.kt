...
interface LeaderBoardRepository : PagingAndSortingRepository<BotDAO?, Long> {
    ...
    @Query("""SELECT new at.tuwien.ase.cardlabs.management.controller.model.LeaderBoardEntry((SELECT count(*) FROM BotDAO c WHERE c.eloScore > b.eloScore ), b.eloScore, b.name, b.owner.username)
            FROM BotDAO b ORDER BY b.eloScore DESC""")
    fun getLeaderBoardEntriesGlobal(pageable: Pageable): Page<LeaderBoardEntry>

    @Query("""SELECT new at.tuwien.ase.cardlabs.management.controller.model.LeaderBoardEntry((SELECT count(*) FROM BotDAO c LEFT JOIN c.owner o1 LEFT JOIN o1.location l1 WHERE c.eloScore > b.eloScore AND  l1.continent = :continent), b.eloScore, b.name, b.owner.username)
    ...
    @Query("""SELECT new at.tuwien.ase.cardlabs.management.controller.model.LeaderBoardEntry((SELECT count(*) FROM BotDAO c LEFT JOIN c.owner o1 LEFT JOIN o1.location l1 WHERE c.eloScore > b.eloScore AND  l1.name = :country), b.eloScore, b.name, b.owner.username)
