...
/**
 * A rule that checks if the given string is not empty
 */
class NotEmptyRule(private var inputName: String?) : ValidationRule<String> {
    ...
    override fun validate(field: String) {
        if (field.isEmpty() || field.isBlank()) {
            throw ValidationException("The $inputName can not be empty or blank")
        }
    }
}
